#!/usr/bin/env julia
# ============================================================================
# bitdollar_crypto_live.jl — BITDOLLAR on the REAL crypto rail (governed).
#
# The validated crypto-trend edge (research/bitdollar_crypto_validation.py: OOS +0.72 Sharpe /
...
# Crypto trades 24/7 (annualize 365). Run:  julia --project=engine live/bitdollar_crypto_live.jl
...
function main(; capital = nothing, pool = "crypto", limits::SafetyLimits = SafetyLimits(),
              db_path     = get(ENV, "BB_LEDGER_PATH", joinpath(REPO, "alpaca_ledger_bitdollar_crypto.sqlite")),
              audit_path  = get(ENV, "BB_AUDIT_PATH",  joinpath(REPO, "alpaca_audit_bitdollar_crypto.jsonl")),
              hwm_path    = get(ENV, "BB_HWM_PATH",    joinpath(homedir(), ".config", "blaquebaux", "equity_hwm_bitdollar_crypto.txt")),
              equity_path = get(ENV, "BB_EQUITY_PATH", joinpath(homedir(), ".config", "blaquebaux", "equity_last_bitdollar_crypto.txt")))
    ...
    if dryrun
        @info "BITDOLLAR-CRYPTO dry run" asof=panel.asof gross=round(bk.gross, digits=2)
    ...
    @info "bitdollar_crypto_live starting" mode; live && alert("BITDOLLAR-CRYPTO LIVE REAL-MONEY mode engaged"; level = :critical)
    ...
    try
        ...
        res = execute_rebalance_frac!(ctrl, ledger; targets = bk.targets, prices = bk.prices, signal_id = "bitdollar_crypto",
        ...
        @info "bitdollar_crypto_live complete" summary; alert(summary; level = :info); return res.reconciled ? :ok : :reconcile_failed
    finally
